...
def fetch_cameras_paginated(roadway: str, max_expected: int = 1000) -> List[Dict]:
    """
    Fetch all cameras for a roadway using pagination
    
    Args:
        roadway: "I-4", "I-95", or "I-10"
        max_expected: Maximum expected number of cameras (safety limit)
    
    Returns:
        List of all camera data
    """
    print(f"🚗 Fetching all {roadway} cameras (max expected: {max_expected})...")
    ...
    while len(all_cameras) < max_expected:
        # Calculate how many to fetch this round
        remaining = max_expected - len(all_cameras)
...
def main():
    """Main function to build I-4, I-95, and I-10 camera lists"""
    print("🛣️  Building comprehensive I-4, I-95, and I-10 camera database")
    print("=" * 70)
    ...
    i4_cameras_raw = fetch_cameras_paginated("I-4", 300)  # Slightly higher limit for safety
    ...
    i95_cameras_raw = fetch_cameras_paginated("I-95", 600)  # Slightly higher limit for safety
    ...
    # Wait before next roadway
    print("⏳ Waiting 2 seconds before fetching I-10...")
    time.sleep(2)
    
    # Fetch I-10 cameras (unknown count, start with reasonable limit)
    print("\n📍 Step 3: Fetching I-10 cameras...")
    i10_cameras_raw = fetch_cameras_paginated("I-10", 500)  # Reasonable starting limit
    i10_cameras = process_camera_data(i10_cameras_raw, "I-10")
    all_cameras.extend(i10_cameras)
    
    print(f"✅ I-10 cameras: {len(i10_cameras)}")
    
    # Save individual files
    save_cameras_json(i4_cameras, 'fl511_i4_cameras.json')
    save_cameras_json(i95_cameras, 'fl511_i95_cameras.json')
    save_cameras_json(i10_cameras, 'fl511_i10_cameras.json')
    
    # Save combined files
    save_cameras_json(all_cameras, 'fl511_i4_i95_i10_cameras.json')
    ...
    print(f"I-10 cameras: {len(i10_cameras):3d}")
    ...
    print(f"  📄 fl511_i10_cameras.json - I-10 only ({len(i10_cameras)} cameras)")
    print(f"  📄 fl511_i4_i95_i10_cameras.json - Combined ({len(all_cameras)} cameras)")
